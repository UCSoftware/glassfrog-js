...
var getMethods = function (context) {
		...
		roles: function() {
			return {
				...
				all: function(cache) {
					...
					// update cache:
					allCache['roles'] = that.sendGET('roles');
					return allCache['roles'];
				}
			};
		},
...
module.exports = function(key) {
	return {
		...
		get: function() {
			...
			return {
				// .get().circles()
				circles: methods.circles,
				// .get().roles()
				roles: methods.roles,
				// .get().people()
				people: methods.people,
				// .get().projects()
				projects: methods.projects,
				// .get().metrics()
				metrics: methods.metrics,
				// .get().checklistItems()
				checklistItems: methods.checklistItems,
				// .get().actions()
		...
		post: function() {
			return {
				...
				checklistItems: function(form) {
					return that.sendPOST({'checklist_items': {
						'checklist_items' : [
							form
						]
					}});
