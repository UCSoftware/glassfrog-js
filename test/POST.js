...
module.exports = function(key) {
	...
	describe('POST', function() {
		...
		gf.get().circles().all().spread(function (response, body) { 
			JSON.parse(body).circles.forEach(function (circle) { 
		...
		gf.get().roles().all().spread(function (response, body) { 
			JSON.parse(body).roles.forEach(function (role) { 
		...
		describe('people', function() {
			it('Should return 200 OK and the object', function(done) {
				gf.post().people(PERSON).spread(function (response, body) {
		...
		describe('projects', function() {
			it('Should return 200 OK or 404 Not Found', function(done) {
				if (testRoleID && testCircleID) {
					...
					gf.post().projects(PROJECT).spread(function (response, body) {
						if (response.headers.status === '200 OK') {
							done();
						} else {
							throw new Error('Not Ok. ' + response.headers.status);
		...
		describe('metrics', function() {
			it('Should return 200 OK or 404 Not Found', function(done) {
				if (testRoleID && testCircleID) {
					...
					gf.post().metrics(METRIC).spread(function (response, body) {
						if (response.headers.status === '200 OK') {
							done();
						} else {
							throw new Error('Not Ok. ' + response.headers.status);
		...
		describe('checklist', function() {
			it('Should return 200 OK or 404 Not Found', function(done) {
				if (testRoleID && testCircleID) {
					...
					gf.post().circles(CHECKLIST_ITEM).spread(function (response, body) {
						if (response.headers.status === '200 OK') {
							done();
						} else {
							throw new Error('Not Ok. ' + response.headers.status);
